...
~/.claude/projects  ─┐
~/.codex            ─┼→ 사용량 키만 → window.pitwallIngest(vendor, lines[])
~/.grok             ─┤                 ↓
~/.copilot          ─┘          LiveSource → 기존 파서
```

- **파싱은 Swift가 안 한다.** 파서를 두 벌 두면 두 화면이 다른 사실을 말한다
- 처음 보는 파일은 **최근 15분**만 넘긴다. 끝만 찍으면 켜기 전과, 한 시간 넘게 쉬었다가 온 첫 묶음이 사라진다. 15분보다 오래된 줄은 앱을 켤 때 쏟아지지 않는다
- 개행 전 완전한 JSON은 한 번만 넘긴다. 개행이 붙어도 같은 줄을 다시 세지 않는다
- 웹뷰로 가는 줄은 파서가 읽는 사용량 키만 남긴다. JSON이 아닌 줄은 버린다
- 계정 식별자는 `accountUuid` / `account_id`만 읽는다. 이메일·토큰은 안 읽는다
- 화면 도달 전 해시. `live:check`가 원문 유출 없음을 검사한다

Claude 한도는 로그에 없다. 빌드 시점 스냅샷을 실시간 게이지에 붙이지 않는다. 로그가 한도를 주면(Codex·Grok) 그 값만 쓰고, 없으면 게이지를 그리지 않는다.

캐시 재전송(`cache_read`)은 랩 거리에서 빼서 따로 센다. 캐시 쓰기(`cache_creation`)는 Claude 로그에만 있고 작업 토큰에 포함된다. 다른 벤더 로그에 없는 값을 0으로 채우지 않는다.

Copilot은 `~/.copilot`을 따라가지만 세션이 끝날 때의 집계다. 호출 한 건이 아니다.
...
- **Claude 누적 행을 tick마다 다시 더함** — 같은 `message.id`는 증가분만 낸다. 캐시 읽기도 그 증가분이다 (`03f8e2a`)
- **Grok 루프와 턴 종료를 둘 다 더함** — 턴 종료는 이미 센 루프를 빼고, 같은 턴 줄은 한 번이다 (`03f8e2a`)
