 ...
 * 한 바퀴에 해당하는 누적 토큰.
 *
 * 실측(2026-07-30): 호출당 프롬프트 중앙값이 139,120 토큰이다 — 캐시 재전송이
 * 대부분이다. 20만으로 두면 한두 번 호출에 한 바퀴를 돌아 "달린 거리"가 의미를 잃는다.
 * 차량당 총 토큰 중앙값(1억)을 8시간에 수십 바퀴로 접는 값으로 맞춘다.
 */
export const LAP_TOKENS = 5_000_000;
