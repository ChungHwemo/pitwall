...
- 역주행 버그: `Projector`가 후퇴 앵커를 전진 속도 상태에 반영하지 않도록 수정하고, 대형 전진 점프·랩 경계·반복 보정 회귀 테스트를 추가했다. 커밋 `9c9f1a6`.
- 한도 차량: `HOT_CAP`을 초과해도 한도 차량을 `cold` 트랙으로 내리지 않도록 변경했다. 핀보다 한도 판정 우선. 커밋 `9c9f1a6`.
- 유휴 이동 실험: stale 이후 진행률을 직접 증가시키는 방식은 서킷 진행 방향을 우회해 반대 방향 이동을 만들 수 있어 폐기했다.

### 2026-08-05 (재검증 · Codex 다듬기 교차확인)

- `Projector` 내부를 누적 진행률로 정리(폐곡선 `shortest` 제거, 반환 시에만 `wrap`). `projection.test.ts` 비-null 단언 제거 + 반 바퀴 이상 전진·다중 랩 테스트 추가. **미커밋.**
- 단일 오프라인 HTML 계약 유지. 코드 분할 기각. `vite.config.ts` `chunkSizeWarningLimit: 8500` — 실기록 포함 7.83MB + 여유. **미커밋.**
- 재측정: `npm test` **857/857** · `tsc --noEmit` 0 · `appMotion` 통과 · `build:single` 5448.3 kB · `build:real` 단일 HTML 7826.9 kB · 경고 없음.
- 잔여: #12 native LIVE 운영 검증 INCONCLUSIVE · #14 렌더러 피트 경로/다수 한도 soft-cap · `projection.test.ts` LOC 분리 권고 · README/PITWALL 테스트 수(847) 미동기화.

### 2026-08-08 (인수인계 완료)

- #13: projection 회귀를 lap/motion/retention/reverse로 분리했다. 커밋 `cd7075c`.
- #14: 다수 한도 차량의 피트 레인/박스 확장과 limit/error 차량의 느리고 결정론적인 피트 이동을 완료했다. 커밋 `da2d646`.
- 리뷰 보강: worst circuit의 다중-ring 비인접 슬롯까지 모든 anchor 사이에 표식 30px + 왕복 여유를 보장했다. 커밋 `3b807c1`.
- 검증: 전체 **53 files / 858 tests**, `tsc --noEmit` 0, `build:single` 5,449.1 kB. headed Chromium `demo-large`에서 stopped 차량 5대의 transform이 1초 사이 모두 바뀌고 콘솔 오류가 없었다.
- native LIVE는 별도 blocker다. `.omo/evidence/live-reload-reliability/task-3-native-reload.md`의 Web Inspector 비활성/BLOCKED 및 운영 연속성 INCONCLUSIVE 판정을 유지하며, 이번 피트 작업이 해결했다고 주장하지 않는다.
