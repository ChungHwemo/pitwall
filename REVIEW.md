...
| 13 | bart | 2026-08-04 | 트랙 진행률 (`pitwall/src/render/projection.ts` · `pitwall/tests/projection.test.ts`) | 앵커 보정 뒤 차가 전진·후진을 반복한다. | 재현 테스트에서 진행률 `0.30 → 0.02`로 큰 후퇴 보정한 뒤 동일 앵커 프레임이 `0.204 → 0.204`로 멈췄다. `Projector.step()`이 후퇴 앵커를 현재 상태로 채택하고 lead clamp가 전진 속도를 매 프레임 상쇄했다. 후퇴 샘플은 상태를 오염시키지 않고 마지막 전진 앵커를 유지하도록 수정했으며, 정상적인 큰 전진 점프와 랩 경계도 보존했다. 전체 854 테스트·빌드 및 adversarial 시퀀스 검증 완료. | P1 | 반영 (미커밋 — 2026-08-04) |
| 14 | bart | 2026-08-04 | 한도 차량 (`pitwall/src/track/trackModel.ts` · `pitwall/src/render/trackRenderer.ts`) | 한도에 걸린 차량은 트랙에 남지 말고 피트로 들어가야 한다. | 한도 차량이 `HOT_CAP`을 초과하면 `cold` 레인으로 밀려 트랙 위에 남을 수 있었다. `limit` 후보를 hot 상한보다 먼저 보존하고, 핀보다 한도 판정을 우선해 모든 한도 차량이 피트 렌더 경로로 가도록 수정했다. 회귀 테스트 통과. | P1 | 반영 (미커밋 — 2026-08-04) |
...
- 역주행 버그: `Projector`가 후퇴 앵커를 전진 속도 상태에 반영하지 않도록 수정하고, 대형 전진 점프·랩 경계·반복 보정 회귀 테스트를 추가했다. 관련 투영 테스트 18건 통과.
- 한도 차량: `HOT_CAP`을 초과해도 한도 차량을 `cold` 트랙으로 내리지 않도록 변경했다. 핀 상태보다 한도 판정을 우선해 모든 한도 차량이 피트 렌더 경로로 들어간다. 관련 트랙 모델·렌더러 테스트 통과.
- 유휴 이동 실험: stale 이후 진행률을 직접 증가시키는 방식은 서킷 진행 방향을 우회해 반대 방향 이동을 만들 수 있어 폐기했다. 유휴 이동 정책은 별도 결정 없이 추가하지 않는다.
- 검증: 프로덕션 빌드 성공. 전체 테스트는 기존 `appMotion` 모션 기대치와 장시간 DOM 테스트 타임아웃 2건이 남아 있으며, 이번 역주행·한도 수정과는 별개다.
