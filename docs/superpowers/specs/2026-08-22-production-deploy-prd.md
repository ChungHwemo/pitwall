...
- `verify`: 모든 push/PR. `pitwall/`에서 `npm install` · `npm test` · `npx tsc --noEmit` · `npm run build:single` · `npm run release:check` · `npm run release:stage`. 스테이징 디렉터리를 일반 artifact로 올린다.
