...
- Produces: `WorkdayConfig` 인터페이스, `DEFAULT_WORKDAY` 상수, `phaseAt(now: Date, cfg: WorkdayConfig): RacePhase`, `elapsedMs(now: Date, cfg: WorkdayConfig): number`
...
```typescript
import { describe, it, expect } from 'vitest';
import { phaseAt, elapsedMs, DEFAULT_WORKDAY } from '../src/state/clock';
...
describe('elapsedMs', () => {
  ...
  it('체커기 이후에는 총 길이에서 멈춘다', () => {
    expect(elapsedMs(at(23, 0), DEFAULT_WORKDAY)).toBe(9 * 3_600_000);
  });
});
```
...
export function elapsedMs(now: Date, cfg: WorkdayConfig): number {
  const m = minutesOfDay(now);
  const total = (cfg.raceEnd - cfg.raceStart) * 60_000;
  if (m < cfg.raceStart) return 0;
  const elapsed = (m - cfg.raceStart) * 60_000;
...
describe('earnedSoFar', () => {
  ...
  it('정확히 절반 지점에서 하루치의 절반이다', () => {
    const perDay = CFG.annualSalary / CFG.workdaysPerYear;
    expect(earnedSoFar(CFG, DEFAULT_WORKDAY, at(13, 30))).toBeCloseTo(perDay / 2, 2);
  });
...
import { elapsedMs } from '../state/clock';
...
export function earnedSoFar(cfg: SalaryConfig, workday: WorkdayConfig, now: Date): number {
  const totalMs = (workday.raceEnd - workday.raceStart) * 60_000;
